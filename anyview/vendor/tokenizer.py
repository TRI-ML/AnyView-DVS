...
# NOTE: the checkpoint loads with torch.load on every rank (no easy_io / rank-0 broadcast),
# and the optional mean/std files are not supported. BenchmarkTimes is inlined from
# cosmos_predict2/utils/tokenizer_benchmarking.py.
...
from . import log
from .tokenizer_interface import VideoTokenizerInterface
...
def _video_vae(
    ...
    **kwargs,
    ...
    assert not load_mean_std, "mean/std files are not vendored (load_mean_std must be False)"
    del mean_std_path
    ...
    if pretrained_path is None:
        ...
        # load checkpoint (on every rank; no rank-0 broadcast)
...
class TokenizerInterface(VideoTokenizerInterface):
    def __init__(self, chunk_duration: int = 81, load_mean_std=False,
                 vae_pth='', temporal_window=4, **kwargs):
        self.model = VAE(
