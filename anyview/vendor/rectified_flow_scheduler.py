...
# NOTE: standalone rewrite of the diffusers-based scheduler. Upstream only used
# diffusers for register_to_config / self.config plumbing; set_timesteps and step are verbatim.
@dataclass
class RectifiedFlowSchedulerConfig:
...
class RectifiedFlowAB2Scheduler:
    def __init__(
        self,
        sigma_min: float = 0.002,
        sigma_max: float = 80.0,
        ...
        self.config = RectifiedFlowSchedulerConfig(
            sigma_min=sigma_min,
            sigma_max=sigma_max,
            order=order,
    ...
    def set_timesteps(self, num_inference_steps, device=None, num_train_timesteps: int | None = None):
        ...
        # Extract values from config for consistency
    ...
    def step(
        ...
        """
        dtype_target = sample.dtype  # float32
        dtype_work = torch.float64 if self.config.use_double_precision else sample.dtype  # float64

        x_t = sample.to(dtype_work)  # (B, C, T, H, W) tensor of float64.
        x0_t = x0_pred.to(dtype_work)  # (B, C, T, H, W) tensor of float64.

        sigma_t = self.sigmas[i]  # single float64.
        sigma_s = self.sigmas[i + 1]  # single float64.

        # Optional stochastic augment (churn) could be added here (currently not handled)

        ones = torch.ones(x_t.shape[0], device=x_t.device, dtype=dtype_work)  # (B) tensor of float64.

        if x0_prev is None:
            # First step - Euler in x0-formulation.
            x_next, _ = reg_x0_euler_step(
                x_t,  # (B, C, T, H, W) tensor of float64.
                sigma_t * ones,  # (B) tensor of float64.
                sigma_s * ones,  # (B) tensor of float64.
                x0_t,  # (B, C, T, H, W) tensor of float64.
            )
        else:
            ...
            x_next = res_x0_rk2_step(
                ...
                sigma_t * ones,  # (B) tensor of float64.
                x0_t,  # (B, C, T, H, W) tensor of float64.
                self.sigmas[i - 1] * ones,  # previous sigma; (B) tensor of float64.
                x0_prev,  # (B, C, T, H, W) tensor of float64.
            )
